...
async function fetchAndApply(request) {
  ...
  if (pathname.startsWith("/app") && pathname.endsWith("js")) {
    ...
    return response;
  } else if (slugs.indexOf(pathname.slice(1)) > -1) {
    const pageId = SLUG_TO_PAGE[pathname.slice(1)];
...
class MetaRewriter {
  element(element) {
    const isEq = (attr, value) => element.getAttribute(attr) === value;
    const set = (attr, value) => element.setAttribute(attr, value);

    const isTitle =
      isEq("property", "og:title") || isEq("name", "twitter:title");

    const isDesc =
      isEq("name", "description") ||
      isEq("property", "og:description") ||
      isEq("name", "twitter:description");

    const isUrl = isEq("property", "og:url") || isEq("name", "twitter:url");

    if (isTitle) set("content", SETTINGS.PAGE_TITLE);
    if (isDesc) set("content", PAGE_DESCRIPTION);
    if (isUrl) set("content", DOMAIN);
  }
}
